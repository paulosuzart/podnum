...
use crate::config::PodNumArgs;

use crate::podnum::server::KVCommand;
...
impl Network {
    fn get_my_api_addr(pid: u64) -> String {
        format!("net:800{}", pid)
    }

    fn get_peer_addr(pid: &u64, receiver_pid: u64) -> String {
        format!("net:80{}{}", pid, receiver_pid)
    }
    ...
    pub(crate) async fn new(args: &PodNumArgs) -> Self {
        ...
        for pid in &peers {
            peer_addrs.insert(*pid, Self::get_peer_addr(&args.pid, *pid));
        }
        let api_stream = TcpStream::connect(Self::get_my_api_addr(args.pid)).await.unwrap();
